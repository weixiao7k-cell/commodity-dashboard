...
var App = (function () {
    ...
    function init() {
        try {
            ...
            if (ts) {
                ...
                // 顶部徽章
                var ub = document.getElementById('updateTime');
                if (ub) ub.textContent = mo + '/' + dd + ' ' + hh + ':' + mm;
                // 底部状态栏
                var el = document.querySelector('.data-source-label');
                if (el) el.textContent = '数据来源: 腾讯自选股 | 最后更新: ' + d.toLocaleString('zh-CN');
            }
    ...
    function renderCharts(code) {
        ...
        // 清除可能的"无数据"提示
        var el = document.getElementById('mainChart');
        if (el && el.children.length > 0 && el.children[0].tagName === 'DIV' && el.children[0].style.display) {
            Charts.disposeChart('mainChart');
            el.innerHTML = '';
        }
        ...
        if (lastBar) {
            var pv = document.getElementById('priceValues');
            if (pv) {
                var config = CommodityData.getCommodityConfig(code);
                var d = config ? (config.decimals || 2) : 2;
                var chg = prevBar ? (lastBar.close - prevBar.close) : 0;
                var chgPct = prevBar && prevBar.close > 0 ? ((chg / prevBar.close) * 100) : 0;
                var cls = chg >= 0 ? 'text-up' : 'text-down';
                var arr = chg >= 0 ? '▲' : '▼';
                pv.innerHTML = '最新: <span class="' + cls + '">' + CommodityData.formatNumber(lastBar.close, d) + '</span> ' + arr + ' <span class="' + cls + '">' + Math.abs(chgPct).toFixed(2) + '%</span>';
            }
        }
    ...
    function bindEvents() {
        ...
        document.getElementById('periodSwitcher').addEventListener('click', function(e) {
            ...
            if (btn) {
                ...
                if (sub) {
                    var labels = { '1m': '分时走势', '1d': '日K线', '1w': '周K线', '1M': '月K线', '1q': '季K线', '1y': '年K线' };
                    sub.textContent = labels[state.currentPeriod] || '日K线';
                }
        ...
        // 指标开关
        var tMA = document.getElementById('toggleMA');
        if (tMA) tMA.addEventListener('change', function(e) { state.showMA = e.target.checked; renderCharts(state.currentCommodity); });
        var tBoll = document.getElementById('toggleBoll');
        if (tBoll) tBoll.addEventListener('change', function(e) { state.showBoll = e.target.checked; renderCharts(state.currentCommodity); });
        var tVol = document.getElementById('toggleVolume');
        if (tVol) tVol.addEventListener('change', function(e) { state.showVolume = e.target.checked; renderCharts(state.currentCommodity); });
