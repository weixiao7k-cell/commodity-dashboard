...
var Charts = (function () {
    ...
    function renderKline(chart, data, opt) {
        ...
        // 计算图表实际宽度，让grid比例正确
        var cw = chart.getWidth() || 735;
        var ch = chart.getHeight() || 420;
        var leftPad = 10, rightPad = 60, topPad = 28, bottomPad = 60;

        var series = [{
            name: '收盘价', type: 'line', data: closes, smooth: false, symbol: 'none',
            lineStyle: { width: 2, color: '#3b82f6' },
            z: 5,
        }];

        if (showMA) {
            series.push({ name: 'MA5', type: 'line', data: Indicators.calcMA(data, 5), smooth: false, symbol: 'none', lineStyle: { width: 1.2, color: '#f59e0b' } });
            series.push({ name: 'MA10', type: 'line', data: Indicators.calcMA(data, 10), smooth: false, symbol: 'none', lineStyle: { width: 1.2, color: '#3b82f6' } });
            series.push({ name: 'MA20', type: 'line', data: Indicators.calcMA(data, 20), smooth: false, symbol: 'none', lineStyle: { width: 1.2, color: '#8b5cf6' } });
            series.push({ name: 'MA60', type: 'line', data: Indicators.calcMA(data, 60), smooth: false, symbol: 'none', lineStyle: { width: 1.2, color: '#06b6d4' } });
        }

        if (showBoll) {
            var boll = Indicators.calcBoll(data, 20, 2);
            series.push({ name: 'BOLL上', type: 'line', data: boll.upper, smooth: false, symbol: 'none', lineStyle: { width: 1, color: '#f59e0b', type: 'dashed', opacity: 0.4 } });
            series.push({ name: 'BOLL中', type: 'line', data: boll.middle, smooth: false, symbol: 'none', lineStyle: { width: 1, color: '#3b82f6', type: 'dotted', opacity: 0.4 } });
            series.push({ name: 'BOLL下', type: 'line', data: boll.lower, smooth: false, symbol: 'none', lineStyle: { width: 1, color: '#10b981', type: 'dashed', opacity: 0.4 } });
        }
        ...
        chart.setOption({
            ...
            legend: {
                ...
                itemWidth: 12, itemHeight: 8, itemGap: 6,
            },
            grid: { left: leftPad, right: rightPad, top: topPad, bottom: bottomPad, containLabel: false },
            xAxis: {
                type: 'category', data: dates, boundaryGap: false,
                axisLine: { lineStyle: { color: AXIS } },
                axisTick: { show: false },
                axisLabel: { color: TXT, fontSize: 10, interval: Math.floor(dates.length / 6) },
            ...
            yAxis: {
                ...
                axisLabel: { color: TXT, fontSize: 10, inside: false, margin: 8 },
            },
            dataZoom: [
                { type: 'inside', start: 0, end: 100, xAxisIndex: 0 },
                { type: 'slider', bottom: 6, left: leftPad, right: rightPad, height: 18,
                  start: 0, end: 100, borderColor: AXIS, fillerColor: 'rgba(59,130,246,0.12)',
                  handleStyle: { color: '#3b82f6' }, textStyle: { color: TXT, fontSize: 9 } },
            ],
            series: series,
        }, true);
    }
